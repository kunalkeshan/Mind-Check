...
const DateOrder: React.FC<DateFilterProps> = ({ filter, setFilter }) => {
	...
	const handleDateOrderhange = (
		value: ScoreHistoryFilter['date']['order']
	) => {
		setFilter((prev) => {
			return {
				...prev,
				date: {
					...prev.date,
					order: value as ScoreHistoryFilter['date']['order'],
				},
			};
		});
	};
	...
	return (
		<div className='relative'>
			...
			>
				<Listbox.Button className='w-full relative cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm'>
								...
								{({ selected }) => (
									<>
										...
										>
											{
												dateOrderOptions[
													order as ScoreHistoryFilter['date']['order']
												]
											}
										</span>
										{selected ? (
